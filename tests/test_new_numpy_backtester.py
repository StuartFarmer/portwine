...
class TestComparisonWithOriginalBacktester(unittest.TestCase):
    ...
    def test_equal_weight_strategy_comparison(self):
        """Test equal weight strategy with new backtester."""
        ...
        # Run new backtester
        results = self.original_backtester.run_backtest(
        ...
        # Verify data types
        self.assertIsInstance(results['signals_df'], pd.DataFrame)
        self.assertIsInstance(results['tickers_returns'], pd.DataFrame)
        self.assertIsInstance(results['strategy_returns'], pd.Series)
        self.assertIsInstance(results['benchmark_returns'], pd.Series)
        
        # Verify non-empty results
        self.assertGreater(len(results['signals_df']), 0)
        self.assertGreater(len(results['tickers_returns']), 0)
        self.assertGreater(len(results['strategy_returns']), 0)
        self.assertGreater(len(results['benchmark_returns']), 0)
        
        # Verify all tickers are present in signals
    ...
    def test_benchmark_comparison(self):
        """Test benchmark functionality with new backtester."""
        ...
        # Run new backtester with benchmark
        results = self.original_backtester.run_backtest(
            self.original_equal_strategy,
            start_date=self.start_date,
            end_date=self.end_date,
        ...
        # Verify benchmark returns are reasonable (not all NaN)
        self.assertFalse(results['benchmark_returns'].isna().all())
        
        # Verify benchmark returns have same index as strategy returns
        self.assertTrue(results['benchmark_returns'].index.equals(results['strategy_returns'].index))
        
        # Verify benchmark returns are numeric
        self.assertTrue(pd.api.types.is_numeric_dtype(results['benchmark_returns']))
    ...
    def test_large_scale_comparison(self):
        """Test new backtester with larger datasets."""
        ...
        # Create strategy for all tickers
        ...
        # Run backtester
        results = large_backtester.run_backtest(
            large_strategy,
            start_date=self.start_date,
            end_date="2020-10-01",
            benchmark=equal_weight_benchmark
        ...
        # Verify results are reasonable (not all NaN)
        self.assertFalse(results['strategy_returns'].isna().all())
        self.assertFalse(results['benchmark_returns'].isna().all())
